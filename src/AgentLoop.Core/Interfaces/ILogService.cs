...
public interface ILogService
{
    ...
    Task<List<LogEntry>> GetRecentLogsAsync(int count = 20, int offset = 0, JobStatus? statusFilter = null, bool? favoritesOnly = null);
    Task<List<LogEntry>> SearchLogsAsync(string searchText, int count = 20, JobStatus? statusFilter = null, bool? favoritesOnly = null);
    Task<int> GetJobLogCountAsync(string jobName);
    Task<int> GetRecentLogCountAsync(JobStatus? statusFilter = null, bool? favoritesOnly = null);
    Task ToggleFavoriteAsync(long id, bool isFavorite);
